...
void read_instance()
{
	...
	for (int i = 0; i <= numFPGA; i++)
	{
		...
		if (index != numFPGA + 1)
		{
			cout << "weight_matrix size error! should be " << numFPGA << " actual " << index<<endl;
			exit(-1);
		}
	}
	FIC.close();
...
bool check_result(vector<Net>& Input_Netgroup, int** delta_weight_M)
{
	...
	if (delta_sum > total_physical_links * 0.3) {
		std::cout << "[E3] Total delta change exceeds limit: delta_sum=" << delta_sum
			<< " > limit=" << total_physical_links * 0.3 << endl;//改动规模超限
		return false;
	}
	...
	for (int i = 1; i <= numFPGA; ++i) {
		if (sum_conn[i] > FPGA_max_weight[i]) {
			std::cout << "[E4] Channel capacity exceeded: FPGA " << i
				<< " total=" << sum_conn[i]
				<< " > max=" << FPGA_max_weight[i] << endl;//容量超限
				return false;
		}
	}
	...
	for (int k = 0; k < (int)Input_Netgroup.size(); k++) {
		for (int t = 0; t < (int)Input_Netgroup[k].path.size(); t++) {
			...
			for (int e = 0; e < (int)P.size(); e++) {
				...
				// 1) 有向连续性
				if (u != cur) {
					std::cout << "[E5] Logical path discontinuity in net " << k+1
						<< " sinkIdx " << t << " cur=" << cur
						<< " edge=(" << u << "," << v << ")"<<endl;
					for(auto x:P){
				...
				if (phys <= 0) {
					std::cout << "[E6] Physical edge missing for net " << k+1
						<< " sinkIdx " << t << " edge=(" << u << "," << v << ")"<<endl;
					return false;
				}

				// 3) 成环检查：不能回到此前访问过的任意节点
				if (v >= 1 && v <= numFPGA && visited[v]) {
					std::cout << "[E7] Cycle detected in net " << k+1
						<< " sinkIdx " << t << " revisiting node " << v << endl;
					return false;
				}
			...
			if (cur != g_fpga) {
				std::cout << "[E8] Path does not end at sink for net " << k+1
					<< " sinkIdx " << t << " end=" << cur
					<< " expected=" << g_fpga << endl;
				return false;
			}
		}
	}
...
// ---------- (E) 计算得分（从零重算 + 仅比较最终结果；使用下标for写法） ----------
	...
	for (int k = 0; k < (int)Input_Netgroup.size(); k++) {
		for (int j = 0; j < (int)Input_Netgroup[k].path.size(); j++) {
			...
			for (int m = 0; m < (int)edges.size(); m++) {
				...
				d += 30 + 0.7 * ceil8((double)nets_count[u][v] / (weight_matrix[u][v] + delta_weight_M[u][v]));
			}
	...
	if (recomputed_max_delay != score_max_delay) {
		std::cerr << "[E9]path_delay:error ,the max_dely="
			<< score_max_delay << " true max net_delay=" << recomputed_max_delay << std::endl;
		return false;
	}
	return true;
}
...
{
	int* best_path = new int[numFPGA+1]; //新建一条路径，用于存储新找的最优路径
...
vector<pair<int, int>> single_dijkstra_1(int source_fpga, int sink_fpga, int** net_count_M,vector<int>&forbid)//找最短路径 直接覆盖
{
	...
	for (int iter = 0; iter < numFPGA; iter++)
	{
		...
		for (int v = 1; v <= numFPGA; v++)
		{
			if (!forbid.empty() && v < (int)forbid.size() && forbid[v] && v != sink_fpga)
				continue;
...
std::vector<int> single_dijkstra_withpath(int source_fpga, int sink_fpga, int** net_count_M, const std::vector<char>& forbid)
{
	...
	for (int iter = 0; iter < numFPGA; iter++)
	{
		...
		for (int v = 1; v <= numFPGA; v++)
		{
			if (!forbid.empty() && v < (int)forbid.size() && forbid[v] && v != sink_fpga) continue;
			if (weight_matrix[u][v] + delta_weight_matrix[u][v] == 0) continue;
			if (visited[v]) continue;
			int tmd_ratio = ceil8((double)(net_count_M[u][v] + 1) / (weight_matrix[u][v] + delta_weight_matrix[u][v]));
			if (tmd_ratio > R_max) continue; //判断最大TDM比率约束
			...
			for (auto edge : path[j])
			{
				...
				{ 
					if ((weight_matrix[edge.first][edge.second] + delta_nets_count_M[edge.first][edge.second]) > 0)
						...
						cout << "tunnel removed but route needed! "<<endl;
				}
			}
...
void copy_solution(vector<Net>& S1, vector<Net> S2, int** S1_delta_weight_M, int** S2_delta_weight_M)
{
	...
	{
		cout << "Two solutions' length is not the same!!!!"<<endl;
		exit(-1);
	}
	else if ((int)S1.size() != numNet)
	{
		cout << "Solution's length do not match the netNum!!!!"<<endl;
		exit(-1);
	}
...
void expand_beam_layer_limit_kdepth(int source_fpga, int sink_fpga, Net&target_net ,int** net_count_M, int farm_index, vector<pair<int, int>>& new_path, vector<pair<int, int>>& farm_son_index, vector<int> &on_path, int depth) {
	...
	if (depth >= Max_kdepth) {
		...
		if (cur_path[0].first != source_fpga) { cout << "[the current path is error]"<<endl; return; }
...
void re_route_path(int net_id, int sink_index) //邻域1
{
	...
	for (auto edges : target_path) //遍历路径中的所有边
	{
		...
		{
			cout << "1path_map of Net does not record an arc!!!"<<endl; exit(-1);
		}
		else {
	...
	for (int x = 0; x < retain_vertics; x++)
	{
		...
		if (successor == sink_fpga){continue;}
		on_path[successor] = 1;
	...
	//根据延迟排序
	if (topk_check_delay.size()!=topk_check.size()){cout<<"the topk_check is error"<<endl;}
...
void neighbor_replan2(int net_id, int sink_index)
{
	...
	for (const auto& e : target_path) {
		...
		if (it == target_net.path_map.end()) {
			std::cerr << "path_map missing arc (" << arc.first << "," << arc.second << ")"<<endl;
			// 清理内存后返回失败
	...
	for (const auto& e : Current[net_id].path[sink_index]) {
		...
		if (it == Current[net_id].path_map.end()) {
			std::cout << "2path_map of Net does not record an arc!!!"<<endl;
			for (int r = 0; r <= numFPGA; ++r) delete[] temp_nets_count_matrix[r];
			delete[] temp_nets_count_matrix;
			exit(-1);
		}
	...
	// if (!check_result(Current, delta_weight_matrix))
	// {
	// 	cout << "the current is error2" << endl;
	// 	// copy_solution(Global, Current);
	// 	file_output();
	// 	exit(-1);
	// }
	if (all_net_delay < Global_delay && abs(all_net_delay - Global_delay)>1e-3)
	{
		...
		// bool check = check_result(Global, delta_weight_matrix);
		// if (!check) cout<<"here 2"<<endl;exit(-1);
	}
...
void add_tunnel(int net_index, int sink_index) //邻域3
{
	...
	{
		cout << "add tunnel exceed max! actual: " << total_add << " ,max: " << (numTunnel * 0.3)<<endl;
		exit(-1);
	}
	...
	// if (!check_result(Current, delta_weight_matrix))
	// {
	// 	cout << "the current is error4" << endl;
	// 	// copy_solution(Global, Current);
	// 	file_output();
	// 	exit(-1);
	// }
	...
	if (new_obj < Global_delay && abs(new_obj- Global_delay)>1e-3)
	{
		...
		// if (!check_result(Current, global_delta_weight_matrix))
		// {
		// 	cout << "the current is error3" << endl;
		// 	// copy_solution(Global, Current);
		// 	file_output();
		// 	exit(-1);
		// }
		// bool check = check_result(Global, global_delta_weight_matrix);
		// if (!check) cout<<"here3"<<endl;exit(-1);
	}
	...
	while (((clock() - beginTime) / CLOCKS_PER_SEC) < maxRunTime)
	{
		for (int i = 0; i < 10; i++)
		{
			...
			switch (neighbor_choose)
			{
				...
				case(1):{
					neighbor_replan2(net_index, sink_index);   
					break;}//邻域2 cout<<i << "N2: ";
...
int main(int argc, char** argv)
{
	...
	bool check = check_result(Global, global_delta_weight_matrix);
	if (!check) cout<<"here5"<<endl;exit(-1);
	cout << "nets_count_matrix: " << endl;
	for (int i = 1; i <= numFPGA; i++)
	{
		for (int j = 1; j <= numFPGA; j++)
