...
#define TRMPLAYER 30 //每个温度尝试次数

//***********************************************************//
char* rep;
char nameFinalResult[256];
char nameIterPara[256];
...
vector<Net> Global, Best, Current; // 所有net的数组，Global记录找到的最优解，Cureent记录搜索过程中变化的解
double* net_delay; // 每个net的延时，用于存储找到的全局最优
double* Global_net_delay; // Global_delay对应的net_delay;
double Global_delay, Best_delay;//Global对应的延迟
...
void read_instance()
{
	...
	R_max = 512; //最大TDM比率
	T = 500.0;
	Global_delay = (double)MAXINT;
	Best_delay = (double)MAXINT;
	...
	best_delta_weight_matrix = new int* [numFPGA + 1];
	...
	for (int i = 0; i < numFPGA + 1; i++)
	{
		...
		best_delta_weight_matrix[i] = new int[numFPGA + 1];
		...
		for (int j = 0; j < numFPGA + 1; j++)
		{
			...
			best_delta_weight_matrix[i][j] = 0;
...
// ===== 统计邻域表现（自适应调度）=====
static long long neigh_attempts[4] = { 0,0,0,0 };
static long long neigh_success[4] = { 0,0,0,0 };
static inline int pick_neighborhood_softmax()
{
	// 成功率的平滑估计 + Softmax 采样
	double score[4];
	for (int i = 0; i < 4; i++) {
		double a = (double)(neigh_attempts[i] + 1);
		double s = (double)(neigh_success[i] + 1);
		score[i] = s / a; // 平均接受率
	}
	// 温度化（越接近1越平均），这里取 tau=0.15 稍激进
	double tau = 0.15;
	double exps[4]; double sum = 0.0;
	for (int i = 0; i < 4; i++)
	{
		exps[i] = exp(score[i] / tau);
		sum += exps[i];
	}
...
void initial_single_dijkstra_jitter(int source_node, int sink_node, vector<pair<int, int>>& path, double& delay_value)
{
	int* best_path = new int[numFPGA]; //新建一条路径，用于存储新找的最优路径
	int source_fpga = nodes_FPGA[source_node];//源点对应的FPGA
	int sink_fpga = nodes_FPGA[sink_node]; //终点对应的FPGA
	...
	for (int iter = 0; iter < numFPGA; iter++)
	{
		...
		for (int v = 1; v <= numFPGA; v++)
		{
			if (weight_matrix[u][v] + delta_weight_matrix[u][v] == 0) continue;
			if (visited[v]) continue;
			int tmd_ratio = ceil8((double)(nets_count_matrix[u][v] + 1) / (double)(weight_matrix[u][v] + delta_weight_matrix[u][v]));
			if (tmd_ratio > R_max) continue; //判断最大TDM比率约束

			double random_ratio = static_cast<double>(rand()) / (RAND_MAX + 1.0);
			double jitter = random_ratio;
			jitter = 0;

			double delay = 30 + (0.7 * (double)tmd_ratio) + jitter;
			if (dist[v] > dist[u] + delay)
			{
				dist[v] = dist[u] + delay;
				prev[v] = u;
...
vector<double> calculate_path(vector<int>& topk_check, vector<vector<pair<int, int>>>& top_k_paths, int** temp_nets_count_matrix)
{ // 计算多个单条路径的值，用于邻域1
	vector<double> topk_check_delay;
	for (auto x : topk_check)
	{
		double delay_path = 0.0;
		for (auto route : top_k_paths[x])
		{
			delay_path += 30 + 0.7 * ceil8((double)(temp_nets_count_matrix[route.first][route.second]) / (weight_matrix[route.first][route.second] + delta_weight_matrix[route.first][route.second]));
		}
		topk_check_delay.push_back(delay_path);
	}
	return topk_check_delay;
}
...
void initialize_solution() //初始化
{
	...
	T = 300;
	T0 = 300;
	double current_max_delay = 0;
	for (int x = 0; x < numNet; x++)
	{
		if (net_delay[x] > current_max_delay)
		{
			current_max_delay = net_delay[x];
		}
	}


	Best_delay = (double)MAXINT;
	if (current_max_delay < Best_delay)
	{
		copy_solution(Best, Current, best_delta_weight_matrix, delta_weight_matrix);
		Best_delay = current_max_delay;
		bestTime = (clock() - beginTime) / CLOCKS_PER_SEC;
		nonImproved = 0;
	}

	if (Best_delay < Global_delay)
	{
		...
		Global_delay = current_max_delay;
		bestTime = (clock() - beginTime) / CLOCKS_PER_SEC;
		nonImproved = 0;
	}
	...
	cout << "Initial: " << current_max_delay << " Global: " << Global_delay << " time: " << (clock() - beginTime) / CLOCKS_PER_SEC << " no-improve: " << nonImproved << endl;
...
void initialize_solution_2() //初始化
{
	for (int n = 0; n < numNet; n++)
	{
		auto& net = Current[n];
		net.path.assign((int)net.sink_num, {});
		unordered_set<pair<int, int>, PairHash> used_pairs;
		for (int x = 0; x < net.sink_num; x++)
		{
			int soure = net.source_node;
			int sink = net.sink_nodes[x];
			double delay = 0.0;
			//single_dijkstra(soure, sink, net.path[x], delay);
			initial_single_dijkstra_jitter(soure, sink, net.path[x], delay);
			//更新使用的边记录
			for (auto& edge : net.path[x])
			{
...
//      
//*****************************************************************************************
void expand_beam_layer_limit_kdepth(int source_fpga, int sink_fpga, Net& target_net, int** net_count_M, int farm_index, vector<pair<int, int>>& new_path, vector<pair<int, int>>& farm_son_index, vector<int>& on_path, int depth)
{

	// 使用类似树的结构将路径存进new_delay; 叶节点是包含sink_fpga;on_path用于去环,farm_son_index,在父节点记录子节点索引的数组。
	if (source_fpga == sink_fpga)
	{
		return;
	}
	if (depth >= Max_kdepth)
	{
		...
		if (cur_path.size() == 1 && cur_path[0].first == -1 && cur_path[0].second == -1)
		{
			return;
		}
		if (cur_path[0].first != source_fpga)
		{
			cout << "[the current path is error]" << endl;
			return;
		}
		if (cur_path.back().second != sink_fpga)
		{
			cout << "[the current path is error]2" << endl;
			return;
		}
		int prev = farm_index;
		for (auto& x : cur_path)
	...
	for (int x = 1; x <= numFPGA; x++) // 寻找后继节点
	{
		...
		int tmd_ratio;
		if (target_net.path_map.find(arc_s) != target_net.path_map.end())
		{
			tmd_ratio = ceil8((double)(net_count_M[source_fpga][x]) / (weight_matrix[source_fpga][x] + delta_weight_matrix[source_fpga][x]));
		}
		else
		{
			tmd_ratio = ceil8((double)(net_count_M[source_fpga][x] + 1) / (weight_matrix[source_fpga][x] + delta_weight_matrix[source_fpga][x]));
		}
		if (tmd_ratio > R_max)
		{
	...
	for (auto s_index : sucessor_vertics)
	{
		...
		farm_son_index.push_back({ 0, 0 });
	}
	++depth;
	// 子节点的索引
	int first_child_index = new_path.size() - retain_vertics; // <-- 1. 在递归前保存起始索引
	farm_son_index[farm_index] = { new_path.size() - retain_vertics, new_path.size() - 1 };
	// 子节点确定
	for (int sss = 0; sss < retain_vertics; sss++)
	{
		int son_index = first_child_index + sss;
		int successor_s = sucessor_vertics[sss];
		on_path[successor_s] = 1;
		expand_beam_layer_limit_kdepth(successor_s, sink_fpga, target_net, net_count_M, son_index, new_path, farm_son_index, on_path, depth);
		on_path[successor_s] = 0;
	}
}

void dfs_fill_path(int t_index, vector<pair<int, int>> curr, vector<vector<pair<int, int>>>& k_paths, vector<pair<int, int>>& t_paths, vector<pair<int, int>>& farm_son)
{ // dfs填路径
	bool pushed = false;
	if (t_index)
	{
	...
	if (l == 0 && r == 0)
	{
		k_paths.push_back(curr);
	}
	else
	{
		for (int tt = l; tt <= r; tt++)
		{
			dfs_fill_path(tt, curr, k_paths, t_paths, farm_son);
		}
	}
	if (pushed)
	{
...
//*****************************************************************************************
bool re_route_path(int net_id, int sink_index, int& global_improved) // 邻域1
{

	int neighber_start_time = (clock() - beginTime) / CLOCKS_PER_SEC;
	// ---------- (0) 数据结构与参数设置  ----------
	...
	for (int x = 1; x <= numFPGA; x++) // 寻找后继节点
	{
		...
		if (target_net.path_map.find(arc_s) != target_net.path_map.end())
		{
			tmd_ratio = ceil8((double)(temp_nets_count_matrix[source_fpga][x]) / (weight_matrix[source_fpga][x] + delta_weight_matrix[source_fpga][x]));
		}
		else
		{
			tmd_ratio = ceil8((double)(temp_nets_count_matrix[source_fpga][x] + 1) / (weight_matrix[source_fpga][x] + delta_weight_matrix[source_fpga][x]));
		}
		if (tmd_ratio > R_max)
		{
			sucessor_vertics.pop_back();
	...
	if (retain_vertics <= 0)
	{
		for (int x = 0; x < numFPGA + 1; x++)
		{
			delete[] temp_nets_count_matrix[x];
		}
	...
	int depth = 1;
	new_path.clear();
	farm_son_index.clear();

	// 添加 Index 0 (根节点)
	new_path.push_back({ -1, -1 }); // 根节点是 -1, -1
	farm_son_index.push_back({ 1, retain_vertics });
	for (int x = 0; x < retain_vertics; x++)
	{
		new_path.push_back({ source_fpga, sucessor_vertics[x] }); // 存入边
		farm_son_index.push_back({ 0, 0 });						// 存入对应的子节点占位符
	}

	vector<int> on_path(numFPGA + 1, 0); // <-- 1. 在循环外只创建一次
	on_path[source_fpga] = 1;			 // <-- 2. 源FPGA始终被标记
	for (int x = 0; x < retain_vertics; x++)
	{
		int farm_id = x + 1;
		int successor = sucessor_vertics[x];
		if (on_path[successor] == 1 && successor != sink_fpga)
		{
			continue;
		}
		if (successor == sink_fpga)
		{
			continue;
		}
		on_path[successor] = 1;
		expand_beam_layer_limit_kdepth(successor, sink_fpga, target_net, temp_nets_count_matrix, farm_id, new_path, farm_son_index, on_path, depth);
		on_path[successor] = 0;
	}
	...
	{
		if (top_k_paths[i].back().second != sink_fpga)
		{
			continue;
		}
		...
		{
			if (visited_node.find(top_k_paths[i][j].second) != visited_node.end())
			{
				judge = true;
				break;
			}
			else
				visited_node.insert(top_k_paths[i][j].second);
		}
	...
	// ---------- (3) 在Top-K中随机选一条并更新 -----------
	auto topk_check_delay = calculate_path(topk_check, top_k_paths, temp_nets_count_matrix);
	// 根据延迟排序
	if (topk_check_delay.size() != topk_check.size())
	{
		cout << "the topk_check is error" << endl;
	}
	qsort_vector(topk_check_delay, topk_check, 0, topk_check.size() - 1);

	// // === 用“概率质量阈值”自适应 save_size ===
	// 已有: topk_check_delay、topk_check 已按延迟升序排好
	int k = (int)topk_check_delay.size();
	int qidx = std::min(k - 1, (int)std::ceil(0.2 * k)); // 前20%分位
	double d0 = topk_check_delay[0];
	double span_q = std::max(1e-12, topk_check_delay[qidx] - d0);

	// 自适应温度（高温更探索，低温更贪心）
	double beta_hi = 0.6, beta_lo = 0.2;
	// double beta = beta_hi * (T / T0) + beta_lo * (1.0 - T / T0);
	double tau = 0.2 * span_q;
	...
	for (int i = 0; i < k; ++i)
	{
		double z = std::exp(-(topk_check_delay[i] - d0) / tau);
		prob[i] = z;
		Z += z;
	}
	for (int i = 0; i < k; ++i)
	...
	// 概率质量阈值决定前缀长度：找到最小 s 使累计概率 ≥ P_cut
	double Pcut_lo = 0.85, Pcut_hi = 0.96;
	double P_cut = Pcut_lo + (Pcut_hi - Pcut_lo) * (T / T0); // 高温更保守（覆盖更多质量）
	double acc = 0.0;
	int s = 1;
	for (; s < k; ++s)
	{
	...
	// 限制一个合理区间，避免过小或过大
	const int s_min = 3, s_max = numFPGA;
	int save_size = std::min(std::max(s, std::min(s_min, k)), std::min(s_max, k));

	// // 确定topk的范围，取RATIO
	// int save_size = RATIO * topk_check.size();
	// save_size = max(save_size, 1);
	topk_check.resize(save_size);
	int idx = rand() % ((int)topk_check.size());
	int chosen_id = topk_check[idx];
	if (is_reverse)
	{
		reverse(top_k_paths[chosen_id].begin(), top_k_paths[chosen_id].end());
		for (auto& x : top_k_paths[chosen_id])
		{
	...
	for (auto edges : Current[net_id].path[sink_index]) // 遍历路径中的所有边
	{
		...
		else
		{
			...
			if (Current[net_id].path_map[arc_change] == 0)
			{
				Current[net_id].path_map.erase(arc_change);
			}
		}
	}
	// 修改原路径同时，调整通道数量
	...
	if (all_net_delay < Best_delay && abs(all_net_delay - Best_delay)>1e-3)
	{
		copy_solution(Best, Current, best_delta_weight_matrix, delta_weight_matrix);
		cout << "Best in N1: from " << Best_delay << " to " << all_net_delay << " time: " << (clock() - beginTime) / CLOCKS_PER_SEC << " no-improve: " << nonImproved << " " << T << endl;
		Best_delay = all_net_delay;
		bestTime = (clock() - beginTime) / CLOCKS_PER_SEC;
		nonImproved = 0;
		is_improved = true;
	}
	...
	if (Best_delay < Global_delay && abs(Best_delay - Global_delay)>1e-3)
	{
		copy_solution(Global, Current, global_delta_weight_matrix, delta_weight_matrix);
		std::copy(net_delay, net_delay + numNet, Global_net_delay);
	...
	// cout << "neighber1 finished " <<  " time: " << (clock() - beginTime) / CLOCKS_PER_SEC -neighber_start_time<< endl;
...
//*****************************************************************************************
bool neighbor_replan2(int net_id, int sink_index, int& global_imrpoved)
{
	// ---------- (0) 数据结构与参数设置 ----------
	...
	/*for (int x = 0; x < numNet; x++)
	{
		if (net_delay[x] > candidate_delay)
		{
			double new_net_delay = 0;
			int source_node = Current[x].source_node;
			int source_fpga = nodes_FPGA[source_node];
			for (int y = 0; y < Current[x].sink_num; y++)
			{
				if (x == net_id && y == sink_index) continue;

				if (Current[x].path_delay[y] > candidate_delay)
				{
					int sink_node = Current[x].sink_nodes[y];
					int sink_fpga = nodes_FPGA[sink_node];
					single_dijkstra(source_fpga, sink_fpga, nets_count_matrix, Current[x].path_delay[y]);
				}
				if (Current[x].path_delay[y] > new_net_delay)
					new_net_delay = Current[x].path_delay[y];
			}
		}
	}*/
	...
	if (all_net_delay < Best_delay && abs(all_net_delay - Best_delay)>1e-3)
	{
		cout << "Best in N2: from " << Best_delay << " to " << all_net_delay << " time: " << (clock() - beginTime) / CLOCKS_PER_SEC << " no-improve: " << nonImproved << " " << T << endl;
		copy_solution(Best, Current, best_delta_weight_matrix, delta_weight_matrix);
		Best_delay = all_net_delay;
		is_improved = true;
		nonImproved = 0;
...
bool add_tunnel(int net_index, int sink_index) //邻域3
{
	// ---------- (0) 数据结构与参数设置  ----------
	...
	// ---------- (1) 增加通道阶段  ----------
	random_rule_add_tunnel_2(net_index, sink_index, add_tunnel_num, added_count, temp_fpga_weight, temp_delta_weight_matrix, temp_added_arc_record);
	...
	unordered_set<pair<int, int>, PairHash> is_used;
	vector<pair<int, int>> not_used_added_arc;
	for (auto each : temp_added_arc_record)
		not_used_added_arc.push_back(each);

	while (adjust_count < adjust_num) //随机取消已增加的通道
	{
		if (not_used_added_arc.empty()) break;


		int adjust_idx = rand() % (int)temp_added_arc_record.size();

		while (is_used.find(temp_added_arc_record[adjust_idx]) != is_used.end())
		...
		{
			is_used.insert(adjust_arc);
			auto it = std::remove(not_used_added_arc.begin(), not_used_added_arc.end(), adjust_arc);
			not_used_added_arc.erase(it, not_used_added_arc.end());
			continue;
		}

		double TDM_ratio = (double)MAXINT;
		if (weight_matrix[f_a][f_b] + temp_delta_weight_matrix[f_a][f_b] - 1 > 0)
			TDM_ratio = ceil8((double)nets_count_matrix[f_a][f_b] / (double)(weight_matrix[f_a][f_b] + temp_delta_weight_matrix[f_a][f_b] - 1));
		else if (weight_matrix[f_a][f_b] + temp_delta_weight_matrix[f_a][f_b] - 1 == 0)
			TDM_ratio = (double)MAXINT;
		if (TDM_ratio > R_max)
		{
			is_used.insert(adjust_arc);
			auto it = std::remove(not_used_added_arc.begin(), not_used_added_arc.end(), adjust_arc);
		...
		auto it = std::find(not_used_added_arc.begin(), not_used_added_arc.end(), adjust_arc);
		not_used_added_arc.erase(it);
	...
	random_rule_add_tunnel_2(net_index, sink_index, adjust_count, actual_adjust, temp_fpga_weight, temp_delta_weight_matrix, temp_added_arc_record);

	//cout << "---------------------phase 2: change completed---------------------" << endl;


	//int total_add = numAddedTunnel + added_count - adjust_num + actual_adjust;
	int total_add = numAddedTunnel + added_count - adjust_count + actual_adjust;
	...
	if (judge)
	{
		...
		numAddedTunnel = numAddedTunnel + added_count - adjust_count + actual_adjust;
	...
	if (new_obj < Best_delay && abs(new_obj - Best_delay)>1e-3)
	{
		cout << "Best in N3: " << "from " << Best_delay << " to " << new_obj << " time: " << (clock() - beginTime) / CLOCKS_PER_SEC << " no-improve: " << nonImproved << " " << T << endl;
		copy_solution(Best, Current, best_delta_weight_matrix, delta_weight_matrix);
		Best_delay = new_obj;
		is_improved = true;
		nonImproved = 0;
...
void random_rule_add_tunnel_5(
	int add_tunnel_num,
	int* temp_fpga_weight,
	int** temp_delta_weight_matrix,
	std::vector<std::pair<int, int>>& arc_record,
	std::vector<int>& node_remaining_capacity, // 使用引用，直接修改
	const std::pair<int, int>& max_delay_arc_cand,
	std::vector<std::pair<int, int>>& max_nets_arc_pool, // 传入副本或可修改的池
	std::vector<std::pair<int, int>>& worst_path_edges_pool,
	const std::pair<int, int>& source_sink_arc_cand,
	std::vector<std::pair<int, int>>& all_available_edges_pool,
	std::vector<std::pair<int, int>>& uesd_available_edges_pool
)
{
	if (add_tunnel_num <= 0) return;

	int added_count = 0;
	std::vector<int> available_rule = { 0, 1, 2, 3, 4, 5 };

	// 高效的约束检查 lambda
	auto ok_add_fast = [&](int a, int b) {
		return a > 0 && b > 0 && node_remaining_capacity[a] > 0 && node_remaining_capacity[b] > 0;
		};

	// 移除规则的 lambda
	auto remove_rule = [&](int rid) {
		auto it = std::remove(available_rule.begin(), available_rule.end(), rid);
		if (it != available_rule.end()) {
			available_rule.erase(it, available_rule.end());
		}
		};

	// 高效的 swap-pop_back lambda
	auto swap_pop_back = [](std::vector<std::pair<int, int>>& pool, int idx) {
	...
	while (added_count < add_tunnel_num && !available_rule.empty())
	{
		...
		case 0: // 最差路径的最差边
		{
			a = max_delay_arc_cand.first;
			b = max_delay_arc_cand.second;
			if (ok_add_fast(a, b)) {
				chose_ok = true;
				if (node_remaining_capacity[a] - 1 == 0 || node_remaining_capacity[b] - 1 == 0)
					remove_rule(0); // 此规则在此次调用中永久失效
			}
			else {
				remove_rule(0); // 此规则在此次调用中永久失效
			}
		...
		case 1: // 整体跨net数量最多边
		{
			while (!max_nets_arc_pool.empty()) {
				int idx = std::rand() % max_nets_arc_pool.size();
				auto cand = max_nets_arc_pool[idx];
				a = cand.first; b = cand.second;
				if (ok_add_fast(a, b)) {
					chose_ok = true;
					if (node_remaining_capacity[a] - 1 == 0 || node_remaining_capacity[b] - 1 == 0)
					{
						swap_pop_back(max_nets_arc_pool, idx); // 懒惰剔除
						if (max_nets_arc_pool.empty())
							remove_rule(1);
					}
					break;
				}
		...
		case 2: // 最差路径中随机选边
		{
			while (!worst_path_edges_pool.empty()) {
				int idx = std::rand() % worst_path_edges_pool.size();
				auto cand = worst_path_edges_pool[idx];
				a = cand.first; b = cand.second;
				if (ok_add_fast(a, b)) {
					chose_ok = true;
					if (node_remaining_capacity[a] - 1 == 0 || node_remaining_capacity[b] - 1 == 0)
					{
						swap_pop_back(worst_path_edges_pool, idx); // 懒惰剔除
						if (worst_path_edges_pool.empty())
							remove_rule(2);
					}
					break;
				}
		...
		case 3: // 纯随机加通道（从全局可用池中选）
		{
			while (!all_available_edges_pool.empty()) {
				int idx = std::rand() % all_available_edges_pool.size();
				auto cand = all_available_edges_pool[idx];
				a = cand.first; b = cand.second;
				if (ok_add_fast(a, b)) {
					chose_ok = true;
					if (node_remaining_capacity[a] - 1 == 0 || node_remaining_capacity[b] - 1 == 0)
					{
						swap_pop_back(all_available_edges_pool, idx); // 懒惰剔除
						if (all_available_edges_pool.empty())
							remove_rule(3);
					}
					break;
				}
			...
			if (!chose_ok) remove_rule(3);
			break;
		}
		case 4: // 起点终点加通道
		{
			a = source_sink_arc_cand.first;
			b = source_sink_arc_cand.second;
			if (a != b && ok_add_fast(a, b)) { // 确保源汇不在一个FPGA
				chose_ok = true;
				if (node_remaining_capacity[a] - 1 == 0 || node_remaining_capacity[b] - 1 == 0)
					remove_rule(4);
			}
		...
		case 5: // 纯随机加通道（从已经占用可用池中选）
		{
			while (!uesd_available_edges_pool.empty()) {
				int idx = std::rand() % uesd_available_edges_pool.size();
				auto cand = uesd_available_edges_pool[idx];
				a = cand.first; b = cand.second;
				if (ok_add_fast(a, b)) {
					chose_ok = true;
					if (node_remaining_capacity[a] - 1 == 0 || node_remaining_capacity[b] - 1 == 0)
					{
						swap_pop_back(uesd_available_edges_pool, idx); // 懒惰剔除
						if (uesd_available_edges_pool.empty())
							remove_rule(5);
					}
					break;
				}
		...
		// 2. 更新剩余容量（关键）
		node_remaining_capacity[a]--;
		node_remaining_capacity[b]--;

		// 3. 记录增加的边（用于未来的移除阶段）
		if (a < b) arc_record.emplace_back(a, b);
		else arc_record.emplace_back(b, a);
...
bool add_tunnel_new_1(int net_index, int sink_index, int& global_improved) //邻域3
{
	// ---------- (0) 数据结构与参数设置  ----------
	int add_tunnel_num = (numTunnel * 0.3) - numAddedTunnel; // 增加的通道最大数量
	int added_count = 0; //实际增加的通道数量
	...
	// 临时矩阵和记录
	int** temp_delta_weight_matrix = new int* [numFPGA + 1];
	int* temp_fpga_weight = new int[numFPGA + 1];
	std::vector<std::pair<int, int>> temp_added_arc_record; // 本次调用中所有增加的边
	std::vector<int> node_remaining_capacity(numFPGA + 1);

	for (int x = 0; x < numFPGA + 1; x++)
	{
		temp_fpga_weight[x] = FPGA_weight[x];
		node_remaining_capacity[x] = (x == 0) ? 0 : (FPGA_max_weight[x] - temp_fpga_weight[x]);
		temp_delta_weight_matrix[x] = new int[numFPGA + 1];
		for (int y = 0; y < numFPGA + 1; y++)
			temp_delta_weight_matrix[x][y] = delta_weight_matrix[x][y];
	}
	...
	// 规则 0 池:
	std::pair<int, int> max_delay_arc_cand = { 0, 0 };
	double arc_max_delay = -1e100;
	for (auto arc : max_delay_path) {
		int u = arc.first, v = arc.second;
		int phys = weight_matrix[u][v] + delta_weight_matrix[u][v]; // 使用当前的 delta
		if (phys <= 0) continue;
		double tdm = ceil8((double)nets_count_matrix[u][v] / (double)phys);
		if (tdm > R_max) continue;
		double d = 30.0 + 0.7 * tdm;
		if (d > arc_max_delay) { arc_max_delay = d; max_delay_arc_cand = arc; }
	}

	// 规则 1 池: 全局net数最多的边
	std::vector<std::pair<int, int>> master_max_nets_arc_pool;
	int max_nets_count = 0;
	for (int i = 1; i <= numFPGA; ++i) {
		for (int j = i + 1; j <= numFPGA; ++j) {
			if (nets_count_matrix[i][j] > max_nets_count) {
				max_nets_count = nets_count_matrix[i][j];
				master_max_nets_arc_pool.clear();
				master_max_nets_arc_pool.emplace_back(i, j);
			}
			else if (nets_count_matrix[i][j] == max_nets_count && max_nets_count > 0) {
				master_max_nets_arc_pool.emplace_back(i, j);
			}
		}
	}
	...
	// 规则 2 池: 最差路径上的所有边
	std::vector<std::pair<int, int>> master_worst_path_edges_pool = max_delay_path;

	// 规则 4 池:
	std::pair<int, int> source_sink_arc_cand = { source_fpga, sink_fpga };

	// 规则 3 池: 全局所有当前可增加的边 (O(F^2) 计算)
	unordered_set<pair<int, int>, PairHash> used_arcs;
	for (int x = 0; x < numNet; x++)
	{
		auto net = Current[x];
		for (int y = 0; y < net.sink_num; y++)
		{
			auto path = net.path[y];
			for (auto arc : path)
			{
				int a = arc.first; int b = arc.second;
				pair<int, int> formatted_arc = {};
				if (a < b) formatted_arc = { a,b };
				else formatted_arc = { b,a };
				if (used_arcs.find(formatted_arc) == used_arcs.end())
					used_arcs.insert(formatted_arc);
			}
		}
	}


	std::vector<std::pair<int, int>> master_all_available_edges_pool;
	std::vector<std::pair<int, int>> master_uesd_available_edges_pool;
	master_all_available_edges_pool.reserve(numFPGA * numFPGA / 2);
	for (int i = 1; i <= numFPGA; ++i) {
		if (node_remaining_capacity[i] > 0) {
			for (int j = i + 1; j <= numFPGA; ++j) {
				if (node_remaining_capacity[j] > 0) {
					master_all_available_edges_pool.emplace_back(i, j);
					if (used_arcs.find({ i,j }) != used_arcs.end())
						master_uesd_available_edges_pool.emplace_back(i, j);
				}
			}
		}
	}
	...
	// ---------- (1) 增加通道阶段 (调用新辅助函数) ----------
	{
		// 创建池的本地副本，因为辅助函数会修改它们
		auto pool1 = master_max_nets_arc_pool;
		auto pool2 = master_worst_path_edges_pool;
		auto pool3 = master_all_available_edges_pool;
		auto pool4 = master_uesd_available_edges_pool;
		random_rule_add_tunnel_5(
			add_tunnel_num,
			temp_fpga_weight, temp_delta_weight_matrix,
			temp_added_arc_record, node_remaining_capacity,
			max_delay_arc_cand, pool1, pool2, source_sink_arc_cand, pool3, pool4
		);
		added_count = (int)temp_added_arc_record.size() - (int)added_arc_record.size();
	}
	// (此时, node_remaining_capacity, temp_fpga_weight, temp_delta_weight_matrix 
	//  和 temp_added_arc_record 已经被更新)


	// ---------- (2) 随机调整通道阶段  ----------
	int adjust_num = SDF_MAX(1, (added_count + numAddedTunnel) * ADJUSTRATIO);
	int adjust_count = 0; // 实际移除的数量
	int actual_adjust = 0; // 实际重新增加的数量

	// 计算被占用的新增通道（这步O(N*S*P)可能仍然较慢，但只执行一次）
	std::unordered_set<std::pair<int, int>, PairHash> routed_used_added_arcs;
	for (int x = 0; x < numNet; x++) {
		if (((clock() - beginTime) / CLOCKS_PER_SEC) > maxRunTime) break;
		for (int y = 0; y < Current[x].sink_num; y++) {
			for (auto arc : Current[x].path[y]) {
				...
				if (weight_matrix[a][b] == 0 && temp_delta_weight_matrix[a][b] > 0)
				{
					if (a < b) routed_used_added_arcs.insert({ a,b });
					else routed_used_added_arcs.insert({ b,a });
				}
			}
		}
	}

	std::vector<std::pair<int, int>> removable_added_arcs; // 未被占用的已增加通道
	for (const auto& arc : temp_added_arc_record) {
		if (routed_used_added_arcs.find(arc) == routed_used_added_arcs.end()) {
			removable_added_arcs.push_back(arc);
		}
	}


	// --- 移除 ---
	while (adjust_count < adjust_num && !removable_added_arcs.empty())
	{
		if (((clock() - beginTime) / CLOCKS_PER_SEC) > maxRunTime) break;

		int pool_idx = std::rand() % removable_added_arcs.size();
		auto adjust_arc = removable_added_arcs[pool_idx];

		// 懒惰移除（从可移除池中）
		auto swap_pop_removable = [&]() {
			std::swap(removable_added_arcs[pool_idx], removable_added_arcs.back());
			removable_added_arcs.pop_back();
			};
		...
		// 检查TDM约束
		double TDM_ratio = (double)MAXINT;
		int phys_after_remove = weight_matrix[f_a][f_b] + temp_delta_weight_matrix[f_a][f_b] - 1;

		if (phys_after_remove > 0) {
			TDM_ratio = ceil8((double)nets_count_matrix[f_a][f_b] / (double)phys_after_remove);
		}
		else if (nets_count_matrix[f_a][f_b] > 0) {
			TDM_ratio = (double)MAXINT; // 不允许移除后通道为0但net不为0
		}
		else {
			TDM_ratio = 0; // 通道为0，net也为0，允许
		}

		if (TDM_ratio > R_max) {
			swap_pop_removable(); // 此边不可移除（TDM违规），从候选池中删除
			continue;
		}
		...
		// 关键：恢复容量
		node_remaining_capacity[f_a]++;
		node_remaining_capacity[f_b]++;

		// 从可移除池中移除
		swap_pop_removable();
		// 从总增加记录中移除
		auto it_total = std::find(temp_added_arc_record.begin(), temp_added_arc_record.end(), adjust_arc);
		if (it_total != temp_added_arc_record.end()) {
			std::swap(*it_total, temp_added_arc_record.back());
			temp_added_arc_record.pop_back();
		}
	...
	if (adjust_count > 0)
	{
		// 关键：必须重建规则3的池，因为 node_remaining_capacity 已经改变
		master_all_available_edges_pool.clear();
		for (int i = 1; i <= numFPGA; ++i) {
			if (node_remaining_capacity[i] > 0) {
				for (int j = i + 1; j <= numFPGA; ++j) {
					if (node_remaining_capacity[j] > 0) {
		...
		int re_added_count = 0; // 临时变量，记录实际加了多少

		std::vector<std::pair<int, int>> re_added_record; // 仅用于计数

		random_rule_add_tunnel_5(
			adjust_count, // 尝试加回与移除数量相同的通道
			temp_fpga_weight, temp_delta_weight_matrix,
			re_added_record, node_remaining_capacity, // 传入 re_added_record
			max_delay_arc_cand, pool1, pool2, source_sink_arc_cand, pool3, pool4
		);
		actual_adjust = (int)re_added_record.size();
		// 将 re_added_record 合并回 temp_added_arc_record
		temp_added_arc_record.insert(temp_added_arc_record.end(), re_added_record.begin(), re_added_record.end());
	}
	...
	if (judge)
	{
		...
		added_arc_record.clear(); // 替换为新的记录
		for (auto each : temp_added_arc_record)
			added_arc_record.push_back(each);
	...
	//double final_delay_obj = judge ? *std::max_element(net_delay, net_delay + numNet) : new_obj;
	//bool is_improved = false;
	//if (final_delay_obj < Best_delay && abs(final_delay_obj - Best_delay) > 1e-3)
	//{
	//	cout << "Best in N3-New: " << "from " << Best_delay << " to " << final_delay_obj << " time: " << (clock() - beginTime) / CLOCKS_PER_SEC << " no-improve: " << nonImproved << " " << T << endl;
	//	// 如果接受了才复制，否则Global保持不变
	//	if (judge) {
	//		copy_solution(Best, Current, global_delta_weight_matrix, delta_weight_matrix);
	//		std::copy(net_delay, net_delay + numNet, Global_net_delay);
	//		Best_delay = final_delay_obj;
	//		bestTime = (clock() - beginTime) / CLOCKS_PER_SEC;
	//		nonImproved = 0;
	//	}
	//	else {
	//		// 理论上，如果 new_obj < Global_delay 但 SA 拒绝了，这里不会被触发
	//		// 但如果触发了，说明 SA 接受了一个非更优解（或 new_obj 是 judge=false 时的值）
	//		// 这种情况不应该更新 Global
	//	}
	//	is_improved = (judge && (Global_delay == final_delay_obj)); // 必须是成功接受的
	//}
	//else if (judge) // 接受了，但没有改善 Global
	//{
	//	is_improved = false;
	//	// nonImproved++; // SA接受了，但不算nonImproved
	//	// if (nonImproved > maxnoImproved)
	//	//     maxnoImproved = nonImproved;
	//}
	...
	if (new_obj < Best_delay && abs(new_obj - Best_delay)>1e-3)
	{
		cout << "Best in N3-new: " << "from " << Best_delay << " to " << new_obj << " time: " << (clock() - beginTime) / CLOCKS_PER_SEC << " no-improve: " << nonImproved << " " << T << endl;
		copy_solution(Best, Current, best_delta_weight_matrix, delta_weight_matrix);
		Best_delay = new_obj;
		is_improved = true;
		bestTime = (clock() - beginTime) / CLOCKS_PER_SEC;
	...
	if (Best_delay < Global_delay && abs(Best_delay - Global_delay)>1e-3)
	{
		cout << "Global improved in N3-new: from " << Global_delay << " to " << new_obj << " time: " << (clock() - beginTime) / CLOCKS_PER_SEC << " no-improve: " << nonImproved << " " << T << endl;
		copy_solution(Global, Current, global_delta_weight_matrix, delta_weight_matrix);
		std::copy(net_delay, net_delay + numNet, Global_net_delay);
		Global_delay = new_obj;
		is_improved = true;
...
{
	cout << "Trigger perturb: \n";

	// 随机选择一些net重新布线
	int num_perturb_nets = SDF_MAX(1, numNet * 0.9);
	for (int i = 0; i < num_perturb_nets; i++) {
		int net_idx = rand() % numNet;
		if (Current[net_idx].sink_num == 0) continue;

		for (auto& p : Current[net_idx].path_map)
		{
			auto arc = p.first;
			int a = arc.first; int b = arc.second;
			nets_count_matrix[a][b]--;
		...
		Current[net_idx].path_map.clear();

		// 为该net重新计算路径
		vector<vector<pair<int, int>>> new_paths;
		multi_dijkstra(Current[net_idx].source_node, Current[net_idx].sink_nodes, new_paths);
		Current[net_idx].path = new_paths;

		// 更新累计使用次数
		auto& net = Current[net_idx];
		pair<int, int> arc = {};
		for (auto& edges : net.path)
		{
			for (auto& e : edges)
	...
	//bool check = check_result(Current, delta_weight_matrix);
	//if (!check)
	//	exit(-1);
}

bool reroute_max_nets(int& global_improved)
{
	/*cout << "--------------reroute-------------------\n";*/
	// 取当前延时 Top-q% 的 nets 进行重路由，q在[0.15,0.30]之间随温度与停滞自适应
	double q = 0.15 + 0.15 * min(1.0, (double)nonImproved / 1500.0);
	int k = max(1, (int)round(numNet * q));
	//找到 top-k net
	vector<int> ids(numNet); iota(ids.begin(), ids.end(), 0);
	stable_sort(ids.begin(), ids.end(), [](int a, int b) { return (net_delay[a] > net_delay[b]); });
	ids.resize(k);
	...
	//vector<int> ids;
	//for (int x = 0; x < numNet; x++)
	//	if (abs(net_delay[x] - current_max_delay) < 1e-3)
	//		ids.push_back(x);


	// 对这些 net 的 path_map 从 nets_count 中扣除
	for (int net_idx : ids) {
		for (auto& p : Current[net_idx].path_map) {
			int a = p.first.first, b = p.first.second;
		...
		// 重路由（对全局最拥塞边轻避开：通过dijkstra成本已隐式体现）
		vector<vector<pair<int, int>>> new_paths;
		multi_dijkstra(Current[net_idx].source_node, Current[net_idx].sink_nodes, new_paths);
		Current[net_idx].path = new_paths;

		pair<int, int> arc = {};
		for (auto& edges : Current[net_idx].path) {
			for (auto& e : edges) {
				int u = e.first, v = e.second; if (u > v) arc = { v,u }; else arc = { u,v };
				if (Current[net_idx].path_map.count(arc)) Current[net_idx].path_map[arc]++;
				else Current[net_idx].path_map[arc] = 1;
			}
		}
		for (auto& p : Current[net_idx].path_map) {
			int a = p.first.first, b = p.first.second;
	...
	if (all_net_delay < Best_delay && abs(all_net_delay - Best_delay)>1e-3)
	{
		cout << "Best in N4: from " << Best_delay << " to " << all_net_delay << " time: " << (clock() - beginTime) / CLOCKS_PER_SEC << " no-improve: " << nonImproved << " " << T << endl;
		copy_solution(Best, Current, best_delta_weight_matrix, delta_weight_matrix);
		Best_delay = all_net_delay;
		nonImproved = 0;
	}
	...
	if (Best_delay < Global_delay && abs(Best_delay - Global_delay)>1e-3)
	{
		cout << "Global improved in N4: from " << Global_delay << " to " << all_net_delay << " time: " << (clock() - beginTime) / CLOCKS_PER_SEC << " no-improve: " << nonImproved << " " << T << endl;
		copy_solution(Global, Current, global_delta_weight_matrix, delta_weight_matrix);
		std::copy(net_delay, net_delay + numNet, Global_net_delay);
		Global_delay = all_net_delay;
		nonImproved = 0;
...
void reset_current()
{
	for (int x = 0; x < numNet; x++)
	{
		Current[x].path_map.clear();
		for (int y = 0; y < Current[x].sink_num; y++)
		{
			Current[x].path[y].clear();
			Current[x].path_delay[y] = (double)MAXINT;
		}
		net_delay[x] = (double)MAXINT;
	}
	...
	for (int x = 0; x < numFPGA + 1; x++)
	{
		for (int y = 0; y < numFPGA + 1; y++)
		{
			delta_weight_matrix[x][y] = 0;
			nets_count_matrix[x][y] = 0;
		}
	}
	added_arc_record.clear();
	numAddedTunnel = 0;
...
void optimize_with_beam_search()
{
	...
	int k = 0;
	int c = 1;
	int re_route_count = 0;
	double Tem = T;
	
	int start_no_improved = 0;
	...
	while (((clock() - beginTime) / CLOCKS_PER_SEC) < maxRunTime)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			int neighbor_choose = pick_neighborhood_softmax();


			double n_search_begin = clock();
			switch (neighbor_choose)
			{
			case(0): { success = re_route_path(net_index, sink_index, global_improved);  neigh_attempts[0]++;   break; }//邻域1 cout <<i<< " N1: ";
			case(1): { success = neighbor_replan2(net_index, sink_index, global_improved); neigh_attempts[1]++;  break; }//邻域2 cout<< i << " N2: ";
			case(2): { success = add_tunnel_new_1(net_index, sink_index, global_improved); neigh_attempts[2]++;   break; }//邻域3 cout <<i<< " N3: ";
			case(3): { success = reroute_max_nets(global_improved);  neigh_attempts[3]++; break; }//邻域3 cout <<i<< " N3: ";
			default:break;
			}
			if (success)
			{
				neigh_success[neighbor_choose]++;
				//reroute_max_nets();
			}
		...
		//T = Tem /(k+c);
		//T = Tem / (1 + (2 * log(1 + k)));
		//cout << nonImproved << endl;

		// cout << "no improve: " << nonImproved << " , 2n: " << 2 * n << endl;
		if (nonImproved >= 2 * n)//3*k
		{

			//***********************************************************//
			ofstream allout(nameIterPara, ios::out | ios::app);
			if (allout.is_open())
			{
				allout << "non improve: " << nonImproved << " , re-start count: " << start_count << " , no improve start: " << start_no_improved << " bestTime: " << bestTime<<endl;
				allout.close();
			}
			//***********************************************************//
...
int main(int argc, char** argv)
{
	...
	strcpy(nameIterPara, "../Iter/");
	strcat(nameIterPara, temp);
	strcat(nameIterPara, "_");
	strcat(nameIterPara, rep);
	strcat(nameIterPara, "_terminate_parameter.txt");
	//***********************************************************//
	...
	for (int x = 0; x < numFPGA + 1; x++)
	{
		...
		delete[] best_delta_weight_matrix[x];
	...
	delete[] best_delta_weight_matrix;
