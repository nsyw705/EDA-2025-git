...
//*****************************************************************************************
bool re_route_path(int net_id, int sink_index) // 邻域1
{
	...
	{
		// cout << "Current: " << all_net_delay << endl;
		is_improved = false;
		nonImproved++;
		if (nonImproved > maxnoImproved)
			maxnoImproved = nonImproved;
...
//*****************************************************************************************
bool neighbor_replan2(int net_id, int sink_index)
{
	// ---------- (0) 数据结构与参数设置 ----------
...
bool add_tunnel_new_1(int net_index, int sink_index) //邻域3
{
	// ---------- (0) 数据结构与参数设置  ----------
	int add_tunnel_num = (numTunnel * 0.3) - numAddedTunnel; // 增加的通道最大数量
	int added_count = 0; //实际增加的通道数量
...
void optimize_with_beam_search()
{
	...
	while (((clock() - beginTime) / CLOCKS_PER_SEC) < maxRunTime)
	{
		for (int i = 0; i < n; i++)
		{
			...
			{
			case(0): { success = re_route_path(net_index, sink_index);  neigh_attempts[0]++;   break; }//邻域1 cout <<i<< " N1: ";
			case(1): { success = neighbor_replan2(net_index, sink_index); neigh_attempts[1]++;  break; }//邻域2 cout<< i << " N2: ";
			case(2): { success = add_tunnel_new_1(net_index, sink_index); neigh_attempts[2]++;   break; }//邻域3 cout <<i<< " N3: ";
			case(3): { success = reroute_max_nets();  neigh_attempts[3]++; break; }//邻域3 cout <<i<< " N3: ";
			default:break;
			}
			if (success)
			{
