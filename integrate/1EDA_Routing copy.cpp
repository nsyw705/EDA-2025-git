...
//*****************************************************************************************
bool re_route_path0(int net_id, int sink_index,int &rad) //邻域1
{
	// ---------- (0) 数据结构与参数设置  ----------
...
bool re_route_path(vector<int> &net_total, vector<vector<int>>& sink_set)
{
	int rad=rand()%2;
	bool improved=false;
	for (int i = 0; i < net_total.size(); i++)
	{
		for (int j = 0; j < sink_set[i].size(); j++)
		{
			bool is_improve =re_route_path0(net_total[i], sink_set[i][j],rad);
			if(is_improve){improved=true;}
		}
	}
	return improved;
}
...
void choose_worst_path_1(vector<Net> S, int& net_id, int& sink_id)
{
	int n = 3;
...
void perturb_solution()
{
	...
	// 随机选择一些net重新布线
	int num_perturb_nets = SDF_MAX(1, numNet * 0.8);
...
void optimize_with_beam_search()
{
	...
	while (((clock() - beginTime) / CLOCKS_PER_SEC) < maxRunTime)
	{
		for (int i = 0; i < n; i++)
		{
			...
			if (neighbor_choose == 0)
			{
				choose_worst_path_3(Current, net_total, sink_set);
			}
			else
			{
				choose_worst_path_4(Current, net_index, sink_index);
			}

			//int neighbor_choose= 0;
			//if(neigh_attempts[0] == 0 || neigh_attempts[1] == 0|| neigh_attempts[2] == 0)
			//	neighbor_choose = rand() % 3;
			//else
			//	neighbor_choose = pick_neighborhood_softmax();
			...
			{
			case(0): { success = re_route_path(net_total, sink_set); neigh_attempts[0]++;   break; }//邻域1 cout <<i<< " N1: ";
			case(1): { success = neighbor_replan2(net_index, sink_index); neigh_attempts[1]++;  break; }//邻域2 cout<< i << " N2: ";
			case(2): { success = add_tunnel(net_index, sink_index); neigh_attempts[2]++;  break; }//邻域3 cout <<i<< " N3: ";
			default:break;
			}
			if (success)
			{
