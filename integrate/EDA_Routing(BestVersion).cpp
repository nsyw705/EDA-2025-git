...
// === 全局定义：当前所有 nets_count_matrix[a][b] > 0 的边集合 ===
std::unordered_set<std::pair<int, int>, PairHash> used_edges;
...
// ===== 统计邻域表现（自适应调度）=====
static long long neigh_attempts[3] = { 0,0,0 };
static long long neigh_success[3] = { 0,0,0 };
...
bool re_route_path(int net_id, int sink_index) // 邻域1
{
	...
	topk_check_delay.resize(save_size);
...
bool neighbor_replan2(int net_id, int sink_index)
{
	...
	// cout << "neighber2 finished " <<  " time: " << (clock() - beginTime) / CLOCKS_PER_SEC -neighber_start_time<< endl;
...
void random_rule_add_tunnel(int net_index, int sink_index, int add_tunnel_num, int& added_count, int* temp_fpga_weight, int** temp_delta_weight_matrix, std::vector<std::pair<int, int>>& arc_record)
{
	...
	// 先定义：在 random_rule_add_tunnel / random_rule_add_tunnel_2 内部，ok_add 已存在
	auto choose_case3_balanced = [&](int& out_a, int& out_b)->bool {
		// 1) 先尝试在“当前最差路径”上做利用（轻微偏置），失败再全局“路径均匀”
		const double P_FOCUS = 0.6; // 可调：0.5~0.8
		if (((double)std::rand() / RAND_MAX) < P_FOCUS) {
			const auto& wpath = Current[net_index].path[sink_index];
			int cnt = 0, pick_a = -1, pick_b = -1;
			for (const auto& e : wpath) {
				int a = e.first, b = e.second;
				if (ok_add(a, b)) {
					++cnt;
					if (std::rand() % cnt == 0) { pick_a = a; pick_b = b; }
				}
			}
			if (cnt > 0) { out_a = pick_a; out_b = pick_b; return true; }
		}

		// 2) 全局“路径均匀”：先用蓄水池抽样均匀挑一条『含至少一条可加边』的路径
		int pick_net = -1, pick_sink = -1;
		long long paths_seen = 0;
		for (int ni = 0; ni < numNet; ++ni) {
			int sn = Current[ni].sink_num;
			for (int sj = 0; sj < sn; ++sj) {
				const auto& path = Current[ni].path[sj];
				bool has_ok_edge = false;
				for (const auto& e : path) {
					if (ok_add(e.first, e.second)) { has_ok_edge = true; break; }
				}
				if (has_ok_edge) {
					++paths_seen;
					if (std::rand() % (int)paths_seen == 0) { pick_net = ni; pick_sink = sj; }
				}
			}
		}
		if (pick_net < 0) return false; // 没有任何可加边

		// 3) 在选中的那条路径里『均匀』挑一条可加边（再次用蓄水池采样）
		const auto& path = Current[pick_net].path[pick_sink];
		long long cnt = 0; int a = -1, b = -1;
		for (const auto& e : path) {
			int x = e.first, y = e.second;
			if (ok_add(x, y)) {
				++cnt;
				if (std::rand() % (int)cnt == 0) { a = x; b = y; }
			}
		}
	...
	while (added_count < add_tunnel_num && !available_rule.empty()) {
		...
		case 3: { // 路径均匀的随机法（无偏）
			if (choose_case3_balanced(a, b)) chose_ok = true;
			else remove_rule(3);
			break;
		}
...
bool add_tunnel(int net_index, int sink_index) //邻域3
{
	...
	// cout << "neighber3 finished " <<  " time: " << (clock() - beginTime) / CLOCKS_PER_SEC -neighber_start_time<< endl;

	return is_improved;
}

void random_rule_add_tunnel_4(
	int add_tunnel_num,
...
/**
 * @brief 邻域3：优化版“增加-调整通道”
 * * 接口与原 add_tunnel 保持一致。
 * 内部逻辑被重构，以消除昂贵的 N*S*P 遍历。
 */
bool add_tunnel_new(int net_index, int sink_index) //邻域3
{
	// ---------- (0) 数据结构与参数设置  ----------
	int add_tunnel_num = (numTunnel * 0.3) - numAddedTunnel; // 增加的通道最大数量
	int added_count = 0; //实际增加的通道数量
	...
	bool is_improved = false;
	if (final_delay_obj < Global_delay && abs(final_delay_obj - Global_delay) > 1e-3)
	{
		cout << "Global improved in N3-New: " << "from " << Global_delay << " to " << final_delay_obj << " time: " << (clock() - beginTime) / CLOCKS_PER_SEC << " no-improve: " << nonImproved << " " << T << endl;

		// 如果接受了才复制，否则Global保持不变
		if (judge) {
			copy_solution(Global, Current, global_delta_weight_matrix, delta_weight_matrix);
			std::copy(net_delay, net_delay + numNet, Global_net_delay);
			Global_delay = final_delay_obj;
			bestTime = (clock() - beginTime) / CLOCKS_PER_SEC;
			//***********************************************************//
			ofstream allout(nameFinalResult, ios::out | ios::app);
			if (allout.is_open())
...
void perturb_solution()
{
	...
	/*	vector<vector<pair<int, int>>> new_paths;
		...
		for (int x = 0; x < Current[net_idx].sink_num; x++)
		{
			int source = Current[net_idx].source_node;
			int sink = Current[net_idx].sink_nodes[x];
			double temp_delay = 0;
			initial_single_dijkstra_jitter(source, sink, Current[net_idx].path[x], temp_delay);
		}
...
void reroute_max_nets()
{
	...
	if (all_net_delay < Global_delay && abs(all_net_delay - Global_delay)>1e-3)
	{
		cout << "Global improved in reroute: from " << Global_delay << " to " << all_net_delay << " time: " << (clock() - beginTime) / CLOCKS_PER_SEC << " no-improve: " << nonImproved << " " << T << endl;
		copy_solution(Global, Current, global_delta_weight_matrix, delta_weight_matrix);
		std::copy(net_delay, net_delay + numNet, Global_net_delay);
		Global_delay = all_net_delay;
		nonImproved = 0;
...
void optimize_with_beam_search()
{
	...
	while (((clock() - beginTime) / CLOCKS_PER_SEC) < maxRunTime)
	{
		for (int i = 0; i < n; i++)
		{
			...
			case(2): { success = add_tunnel_new(net_index, sink_index); neigh_attempts[2]++;  break; }//邻域3 cout <<i<< " N3: ";
			default:break;
			}
			if (success)
			{
		...
		//check_result(Global, global_delta_weight_matrix);
