...
bool re_route_path(int net_id, int sink_index) // 邻域1
{
	...
	int save=min((int)topk_check.size(),numFPGA);
	topk_check.resize(save);
	int idx1 = rand() % save;
	int idx2 = rand() % save;

	int chosen_idx = (topk_check_delay[idx1] < topk_check_delay[idx2]) ? idx1 : idx2;
	int chosen_id = topk_check[chosen_idx];
