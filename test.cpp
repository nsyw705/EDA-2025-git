...
std::string designNet;
std::string designTopo;
std::string designFpgaOut;
int seed;
double maxRunTime;
double beginTime;
double bestTime;
...
void read_instance()
{
	...
	if (FIC.fail())
	{
		cout << "1-can not open the file " << designInfo << endl;
		exit(0);
	}
	if (FIC.eof())
	{
		cout << "2-can not open the file " << designInfo << endl;
	}
	...
	if (FIC.fail())
	{
		cout << "1-can not open the file " << designNet << endl;
		exit(0);
	}
	if (FIC.eof())
	{
		cout << "2-can not open the file " << designNet << endl;
	}
	...
	if (FIC.fail())
	{
		cout << "1-can not open the file " << designTopo << endl;
		exit(0);
	}
	if (FIC.eof())
	{
		cout << "2-can not open the file " << designTopo << endl;
	}
	...
	FIC.open(designFpgaOut);
	if (FIC.fail())
	{
		cout << "1-can not open the file " << designFpgaOut << endl;
		exit(0);
	}
	if (FIC.eof())
	{
		cout << "2-can not open the file " << designFpgaOut << endl;
	}
...
void file_output()//输出design.route.out
{
	...
	std::ofstream out(outputPath+"/design.route.out", std::ios::out);
	if (!out.is_open())
		return;
	...
	std::ofstream matrixout(outputPath+"/design.newtopo", std::ios::out);
...
{
	if (argc != 5) {
	std::cout << "用法: " << argv[0] << " -input <输入文件夹路径> -output <输出文件夹路径>" << std::endl;
	return 1;
	}
	...
	designInfo    = inputPath  + "/design.info";
	designNet     = inputPath  + "/design.net";
	designTopo    = inputPath  + "/design.topo";
	designFpgaOut = outputPath + "/design.fpga.out";
	seed = 8; // 随机数种子
