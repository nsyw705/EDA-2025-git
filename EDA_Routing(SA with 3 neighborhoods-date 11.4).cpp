...
void re_route_path(int net_id, int sink_index) //邻域1
{
	...
	vector<Net> tempCurrent;
	for (auto net : Current)
		tempCurrent.push_back(net);
	//实际取消原路径：取消原路径，调整通道数量
	pair<int, int> arc_change = {};
	for (auto edges : tempCurrent[net_id].path[sink_index]) // 遍历路径中的所有边
	{
		int a = edges.first; int b = edges.second;
		if (a > b) { arc_change = { b, a }; }
		else { arc_change = { a, b }; }

		if (tempCurrent[net_id].path_map.find(arc_change) == tempCurrent[net_id].path_map.end()) // 在net中查找此边的使用次数
		{
			cout << "2path_map of Net does not record an arc!!!";
			exit(-1);
		}
		else {
			if (tempCurrent[net_id].path_map[arc_change] == 1) // 该边在整个net中仅使用一次，取消原路径后，此net不再跨越此边，对应记录减一
			{
				temp_nets_count_matrix[a][b]--; temp_nets_count_matrix[b][a]--;
			}
			tempCurrent[net_id].path_map[arc_change]--;
			if (tempCurrent[net_id].path_map[arc_change] == 0) { tempCurrent[net_id].path_map.erase(arc_change); }
		}
	}
	//修改原路径同时，调整通道数量
	tempCurrent[net_id].path[sink_index] = top_k_paths[chosen_id];
	...
	for (auto edges : tempCurrent[net_id].path[sink_index]) // 遍历路径中的所有边
	{
		...
		else { tempCurrent[net_id].path_map[arc_add]++; }
	}
	
	temp_net_delay = 0;
	for (int i = 0; i < numNet; i++)
	{
		int sink_num = tempCurrent[i].sink_num;
		auto& path = tempCurrent[i].path;
		for (int j = 0; j < sink_num; j++)
		{
			double each_path_delay = 0;
			if (path[j].size() == 0) { continue; }
			for (auto edge : path[j])
			{
				if (edge.first != edge.second) { each_path_delay += 30 + 0.7 * ceil8((double)temp_nets_count_matrix[edge.first][edge.second] / (weight_matrix[edge.first][edge.second] + delta_weight_matrix[edge.first][edge.second])); }
			}
			if (each_path_delay > temp_net_delay) { temp_net_delay = each_path_delay; }
		}
	}
	//**************************************************************************************************************************************
...
void neighbor_replan2(int net_id, int sink_index)
{
	...
	// 6.1 撤销旧路径（path_map--；当从1->0时 temp_nets_count_matrix 对称-- 并擦除键）
	std::pair<int, int> arc_change;
	for (const auto& e : tempCurrent[net_id].path[sink_index]) {
		int a = e.first, b = e.second;
		if (a > b) arc_change = { b, a }; else arc_change = { a, b };
		auto it = tempCurrent[net_id].path_map.find(arc_change);
		if (it == tempCurrent[net_id].path_map.end()) {
			...
			if (it->second == 1) {
				temp_nets_count_matrix[a][b]--;
				temp_nets_count_matrix[b][a]--;
				tempCurrent[net_id].path_map.erase(it);
			}
			else {
				it->second--;
			}
	...
	// 2) 覆盖写入新路径
	tempCurrent[net_id].path[sink_index] = new_edges; //（或 top_k_paths[chosen_id]，按你的场景）

	// 3) 新路径生效：path_map++；当从0增到1时，temp_nets_count_matrix 对称++
	{
		std::pair<int, int> arc_add;
		for (const auto& edges : tempCurrent[net_id].path[sink_index]) { // 新路径的每条边
			int a = edges.first, b = edges.second;
			if (a > b) arc_add = { b, a }; else arc_add = { a, b };

			auto it = tempCurrent[net_id].path_map.find(arc_add);
			if (it == tempCurrent[net_id].path_map.end()) {      // 0 -> 1：全局跨边计数对称++
				temp_nets_count_matrix[a][b]++;
				temp_nets_count_matrix[b][a]++;
				tempCurrent[net_id].path_map[arc_add] = 1;
			}
	...
	for (int i = 0; i < numNet; i++)
	{
		...
		for (int j = 0; j < sink_num; j++)
		{
			...
			if (each_path_delay > candidate_delay) { candidate_delay = each_path_delay; }
		}
	}
