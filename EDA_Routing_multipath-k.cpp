...
void choose_worst_path_3(vector<Net> &S, vector<int> &net_total, vector<int> &sink_set) // 一条net，net中多个路径
{
	int n = 3;
	double *net_delay_record = new double[numNet];
	vector<vector<int>> max_nets_set;
	...
	while (net_num)
	{
		...
		if(un_net.find(net_id)==un_net.end()){
			net_total.push_back(net_id);
			sink_set.resize(net_total.size(),0);
			...
			int net_pathid=rand()%path_num;
			
			sink_set[net_total.size()-1]=sink_id[sink_id.size()-net_pathid-1];
	...
	while (((clock() - beginTime) / CLOCKS_PER_SEC) < maxRunTime)
	{
		for (int i = 0; i < n; i++)
		{
			...
			case(0): { for(int i=0;i<net_total.size();i++){
									re_route_path(net_total[i], sink_set[i]);
						 }
						break;}//邻域1 cout <<i<< " N1: ";
