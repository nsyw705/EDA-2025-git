...
void choose_worst_path_3(vector<Net> &S, vector<int> &net_total, vector<vector<int>> &sink_set) // 一条net，net中多个路径
{
	...
	while (net_num)
	{
		...
		if(un_net.find(net_id)==un_net.end()){
			...
			vector<double> net_delay=S[net_id].path_delay;
			vector<int> sink_id(S[net_id].sink_num,0);
			for (int i = 0; i < S[net_id].sink_num; i++){sink_id[i]=i; }
			qsort_vector(net_delay,sink_id,0,sink_id.size()-1);
			int path_num=RATIO*sink_id.size();
			// cout<<path_num<<"+"<<endl;
			path_num=max(1,path_num);path_num=min(path_num,numFPGA);
			for(int i=1;i<=path_num;i++){
				sink_set[net_total.size()-1].push_back(sink_id[sink_id.size()-i]);
			}
