...
void choose_worst_path_3(vector<Net> &S, int &net_id, int &sink_id) // 一条net，net中多个路径
{
	int n = 3;
	double *net_delay_record = new double[numNet];
	vector<vector<int>> max_nets_set;
	...
	vector<int> v(union_net.begin(), union_net.end());
	int random_id=rand()%v.size();
	net_id=v[random_id];
	...
	// cout<<path_num<<"+"<<endl;
	path_num = max(1, path_num);
	// path_num = min(path_num, numFPGA);
	int net_pathid = rand() % path_num;
	sink_id=Sink_id[Sink_id.size()-1-net_pathid];
	...
	while (((clock() - beginTime) / CLOCKS_PER_SEC) < maxRunTime)
	{
		for (int i = 0; i < n; i++)
		{
			...
			if(neighbor_choose==0){
				choose_worst_path_3(Current, net_index, sink_index);
			}
			...
			{
			case(0): { re_route_path(net_index, sink_index);
						break;}//邻域1 cout <<i<< " N1: ";
