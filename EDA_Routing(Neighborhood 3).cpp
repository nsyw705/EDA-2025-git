	...
	for (int k = 0; k < (int)Input_Netgroup.size(); ++k) {
		for (int j = 0; j < (int)Input_Netgroup[k].path.size(); ++j) {
			...
			for (int m = 0; m < (int)edges.size(); ++m) {
				...
				d += 30 + 0.7 * ceil8(
					(double)nets_count[u][v] / (double)(weight_matrix[u][v] + delta_weight_matrix[u][v])
				);
			}
...
// 输出：路径、路径延迟
//*****************************************************************************************
double single_dijkstra_value(int source_fpga, int sink_fpga, int** net_count_M, double& delay_value)
{
...
double single_dijkstra_path(int source_fpga, int sink_fpga, int** net_count_M, double& delay_value, vector<pair<int, int>>& path)
{
	int* best_path = new int[numFPGA]; //新建一条路径，用于存储新找的最优路径
...
void calculate_edges(vector<Net>& S)
{
	for (int x = 0; x < numFPGA + 1; x++)
		...
		for (int y = 0; y < S[x].sink_num; y++)
		{
			...
			for (auto& e : S[x].path[y])
			{
				...
				if (u > v)
					...
					arc = { u, v };
				if (S[x].path_map.find(arc) == S[x].path_map.end())
					S[x].path_map[arc] = 1;
				else if (S[x].path_map.find(arc) != S[x].path_map.end())
					S[x].path_map[arc]++;
			}
		}

		for (auto i = S[x].path_map.begin(); i != S[x].path_map.end(); i++)
		{
			pair<int, int> arc = {};
			arc = i->first;
			int a = arc.first;
...
void expand_beam_layer(int source_fpga, int sink_fpga, int** net_count_M, int& farm_index, vector<pair<int, int>>& new_path, vector<pair<int, int>>& farm_son_index, vector<int>& on_path)
{
	...
	for (int x = 1; x <= numFPGA; x++) //寻找后继节点
	{
		...
		if (x != sink_fpga) {
			single_dijkstra_value(x, sink_fpga, net_count_M, path_delay); // 从后继节点到终点的dijkstra路径延时值
...
void re_route_path(int net_id, int sink_index)
{
	...
	for (int x = 1; x <= numFPGA; x++) //寻找后继节点
	{
		...
		double path_delay = (double)MAXINT;
		if (x != sink_fpga) {
			single_dijkstra_value(x, sink_fpga, temp_nets_count_matrix, path_delay); //从后继节点到终点的dijkstra路径延时值
			sucessor_delay[index] += path_delay;
		}
		index++;
	}
...
void route_new_path(int net_id, int sink_index)
{
	//cout << "Re path: net " << net_id << " sink " << sink_index << endl;

	int source_node = Current[net_id].source_node; //net中的起点
	int sink_node = Current[net_id].sink_nodes[sink_index]; // net中的终点
	int source_fpga = nodes_FPGA[source_node]; // 起点、终点对应的FPGA
	int sink_fpga = nodes_FPGA[sink_node];
	...
	for (auto edges : Current[net_id].path[sink_index]) //遍历路径中的所有边
	{
		...
		if (Current[net_id].path_map.find(arc) == Current[net_id].path_map.end()) //在net中查找此边的使用次数
		{
			cout << "1path_map of Net does not record an arc!!!";
			exit(-1);
		}
		else {
			if (Current[net_id].path_map[arc] == 1) //该边在整个net中仅使用一次，取消原路径后，此net不再跨越此边，对应记录减一
			{
				nets_count_matrix[a][b]--;
				nets_count_matrix[b][a]--;
				Current[net_id].path_map.erase(arc);
			}
			else
				Current[net_id].path_map[arc]--;
		}
	}

	single_dijkstra_path(source_fpga, sink_fpga, nets_count_matrix, Current[net_id].path_delay[sink_index], Current[net_id].path[sink_index]);
	...
	for (auto edges : Current[net_id].path[sink_index]) //遍历路径中的所有边
	{
		...
		if (Current[net_id].path_map[arc] == 1) //该边在整个net中仅使用一次，取消原路径后，此net不再跨越此边，对应记录减一
		{
			nets_count_matrix[a][b]++;
			nets_count_matrix[b][a]++;
			//cout << "nets_count_matrix change: " << a << " " << b << " value: " << nets_count_matrix[a][b]<<" "<< nets_count_matrix[b][a]<<endl;
		}
	}
}
...
void beam_search(int beamK = 3, int max_layers = -1, int topK = -1)
{
	...
	while (no_improve <= search_depth) // 主循环
	{
		...
		// ---------- (1) 选“最差路径”和同net的可用连接 ----------
		n = SDF_MAX((int)(4 - (0.5 * (double)no_improve)), 1);
		max_nets_set.clear();
		max_sinks_set.clear();
		...
		//origin_delay = top_net_delays[net_idx];


		// ---------- (2) 束搜索重新规划路径（含可行性校验） ----------
		origin_delay = net_delay[max_net];
		re_route_path(max_net, max_path_sink_index); //重新规划最差路线
		calculate_su(Current);

		double re_route_delay = net_delay[max_net];
		if (re_route_delay < origin_delay)
			no_improve = 0;
		else
			no_improve++;
		if (re_route_delay < Global_delay)
		{
			for (int x = 0; x < numNet; x++)
			{
				if (net_delay[x] > re_route_delay)
				{
					for (int y = 0; y < Current[x].sink_num; y++)
					{
						if (Current[x].path_delay[y] > re_route_delay)
							route_new_path(x, y);
					}
				}
			}
			...
			if (new_max_delay < Global_delay) //&& abs(new_max_delay - Global_delay) > 1e-3
			{
				copy_solution(Global, Current);
				...
				cout << "fail to Global, Current value: " << new_max_delay << endl;
		}
...
void random_rule_add_tunnel(int net_index, int sink_index, int add_tunnel_num, int& added_count, int* temp_fpga_weight, int** temp_delta_weight_matrix, pair<int, int>* added_arc_record)
{
	...
	for (auto arc : max_delay_path)
	{
		...
		if (TDM_ratio > R_max)
		{
			cout << "Input path arc (" << arc.first << ", " << arc.second << ") over TDM max ratio!";
			exit(-1);
		}
	...
	while (added_count < add_tunnel_num)
	{
		...
		cout << added_count << ' ' << add_tunnel_num << ' ' << rule << ' '<<(clock()- check_last_time)/CLOCKS_PER_SEC;
		...
		case(0): //最差路径的最差边加通道
		{
			...
			if (added_count < add_tunnel_num)
				added_arc_record[added_count] = added_arc;
			break;
		}
		...
		for (int x = 0; x < (int)available_nets.size(); x++)
			for (int y = 0; y < (int)available_nets[x].size(); y++)
				for (int z = 0; z < (int)available_nets[x][y].size();)
				{
					auto arc = available_nets[x][y][z];
					int arc_a = arc.first; int arc_b = arc.second;
					if (reach_max_weight[arc_a] || reach_max_weight[arc_b])
						available_nets[x][y].erase(available_nets[x][y].begin() + z);
					else
						z++;
				}

		for (int x = 0; x < (int)available_nets.size(); x++)
			for (int y = 0; y < (int)available_nets[x].size();)
			{
				if (available_nets[x][y].empty())
					available_nets[x].erase(available_nets[x].begin() + y);
				else
					y++;
			}

		for (int x = 0; x < (int)available_nets.size();)
		{
			if (available_nets[x].empty())
				available_nets.erase(available_nets.begin() + x);
			else
				x++;
		}

		if (available_nets.empty())
		{
			auto it = std::remove(available_rule.begin(), available_rule.end(), 3);
			available_rule.erase(it, available_rule.end());
		}
...
void add_tunnel(int net_index, int sink_index) //邻域3
{
	...
	pair<int, int>* added_arc_record = new pair<int, int>[add_tunnel_num]; //增加的通道位置记录
	pair<int, int>* adjust_arc_record = new pair<int, int>[add_tunnel_num]; //更改的通道位置记录
	...
	for (int x = 0; x < add_tunnel_num; x++)
	{
		added_arc_record[x] = { };
		adjust_arc_record[x] = { };
	}
	...
	// ---------- (1) 增加通道阶段  ----------
	random_rule_add_tunnel(net_index, sink_index, add_tunnel_num, added_count, temp_fpga_weight, temp_delta_weight_matrix, added_arc_record);

	cout << "---------------------phase 1: add completed---------------------" << endl;
	// ---------- (2) 随机调整通道阶段  ----------
	
	int adjust_num = added_count * adjust_ratio;
	int adjust_count = 0;
	int actual_adjust = 0;

	while (adjust_count < adjust_num) //随机取消已增加的通道
	{
		int adjust_idx = rand() % added_count;
		auto adjust_arc = added_arc_record[adjust_idx];
		int f_a = adjust_arc.first; int f_b = adjust_arc.second;
		if (f_a < 0 || f_b < 0) continue;
		...
		if(temp_delta_weight_matrix[f_a][f_b] == 0 || temp_delta_weight_matrix[f_b][f_a] == 0)
			added_arc_record[adjust_idx] = { -1,-1 };

		adjust_count++;
	}

	random_rule_add_tunnel(net_index, sink_index, adjust_count, actual_adjust, temp_fpga_weight, temp_delta_weight_matrix, adjust_arc_record);
	...
	if (judge)
	{
		for (int x = 0; x < numFPGA + 1; x++)
			for (int y = 0; y < numFPGA + 1; y++)
			{
				...
				if (x < y && temp_delta_weight_matrix[x][y] > 0)
				{
					FPGA_weight[x] += temp_delta_weight_matrix[x][y];
					FPGA_weight[y] += temp_delta_weight_matrix[x][y];
				}
			}
	}
	...
	{
		cout << "Global improved: " << Global_delay << "to " << new_obj << endl;
		copy_solution(Global, Current);
	...
	delete[] added_arc_record;
	delete[] adjust_arc_record;
...
void optimize_with_beam_search()
{
	...
	while (((clock() - beginTime) / CLOCKS_PER_SEC) < maxRunTime)
	{
		...
		for (int i = 0; i < 1; i++)
		{
			...
			//邻域3
			add_tunnel(net_index, sink_index);
		}
		T = T * 1.0;
	}
...
void file_output()//输出design.route.out
{
	...
	for (int x = 1; x <= numFPGA; x++)
	{
		...
		for (int y = 1; y < numFPGA; y++)
			matrixout << weight_matrix[x][y] + delta_weight_matrix[x][y] << ',';
		matrixout << weight_matrix[x][numFPGA] + delta_weight_matrix[x][numFPGA];
		matrixout << "\n";
	}
	matrixout.close();
}
