...
void re_route_path(int net_id, int sink_index) //邻域1
{
	...
	if (temp_net_delay < Global_delay && abs(temp_net_delay - Global_delay)>1e-3)
	{
		copy_solution(Global, Current, global_delta_weight_matrix, delta_weight_matrix);
		std::copy(net_delay, net_delay + numNet, Global_net_delay);
		cout << "Global improved in N1: from " << Global_delay << " to " << temp_net_delay << " " << (clock() - beginTime) / CLOCKS_PER_SEC << endl;
...
void neighbor_replan2(int net_id, int sink_index)
{
	...
	{
		cout << "Global improved in N2: " << Global_delay << " to " << current_obj << " " << (clock() - beginTime) / CLOCKS_PER_SEC << endl;
		copy_solution(Global, Current, global_delta_weight_matrix, delta_weight_matrix);
		std::copy(net_delay, net_delay + numNet, Global_net_delay);
...
void add_tunnel(int net_index, int sink_index) //邻域3
{
	...
	{
		cout << "Global improved in N3: " << "from " << Global_delay << " to " << new_obj << " " << (clock() - beginTime) / CLOCKS_PER_SEC << endl;
		copy_solution(Global, Current, global_delta_weight_matrix, delta_weight_matrix);
		std::copy(net_delay, net_delay + numNet, Global_net_delay);
